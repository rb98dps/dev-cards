...
public class UserServiceImpl extends GenericServiceImpl<User, UUID> implements UserService {
    ...
    @Override
    @Transactional
    public User save(CreateUserRequest createUserRequest) throws IllegalAccessException, NoSuchMethodException, InstantiationException, InvocationTargetException, CustomDevApiException {
    ...
    public User assignRoles(AssignRoleRequest assignRoleRequest) throws Exception {
        ...
        User user = findByEmailOrId(getUserRequest);
        List<Role> roles = roleService.getRoles(assignRoleRequest.getAssignRoles());
        if (roles.isEmpty()){
            throw new CustomDevApiException(RoleException.ROLE_NOT_FOUND);
        }
        if (user != null) {
            addRolesToUser(user, roles);
        } else {
